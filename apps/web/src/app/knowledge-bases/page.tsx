...
export default function KnowledgeBasesPage() {
  ...
  async function submit() {
    ...
    try {
      if (editing) {
        const res = await fetch(`${getApiBase()}/knowledge-bases/${editing.id}`, {
          method: "PATCH",
  ...
  async function removeRow(id: string) {
    const res = await fetch(`${getApiBase()}/knowledge-bases/${id}`, { method: "DELETE" });
    if (!res.ok) {
    ...
    <div>
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item name="storageHint" label="存储 / 索引提示">
